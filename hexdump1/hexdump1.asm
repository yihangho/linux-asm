...
Scan:
	...
	; Note: cannot copy from memory to memory (mov byte[HexStr+edx+2], byte[Digits+eas]), hence, have to copy to register first
	and al, 0xF
	...
	inc ecx
	cmp ecx, ebp
	jna Scan ; Note: shouldn't it be jb?

	; Write to stdout
	mov eax, 4 ; sys_write
	mov ebx, 1 ; stdout
	mov ecx, HexStr
	mov edx, HEXLEN ; Note: shouldn't it be edx+3?
	int 0x80
	jmp Read
